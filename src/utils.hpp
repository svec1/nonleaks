...
using buffer_bytes_t = std::array<char, buffer_size>;
...
class monotonic_buffer_resource_static final
  ...
  protected:
    void *do_allocate(std::size_t bytes, std::size_t alignment) {
        static constexpr auto calc_alignment_bytes = [](std::size_t offset,
                                                        std::size_t alignment) {
            std::size_t offset_end = offset;
            while (offset_end % alignment)
                offset_end += offset_end % alignment;
            return offset_end - offset;
        };
        const std::size_t bytes_alignment =
            calc_alignment_bytes(offset, alignment);

        offset += bytes + bytes_alignment;

        if (offset >= buffer_size)
            throw runtime_error(
                "The allocator buffer is full: {} bytes were allocated.",
                buffer_size);
...
template <typename T, std::size_t max_count> struct vector_stack {
...
class log_handler {
  ...
  public:
    constexpr log_handler(noheap::log_impl::owner_impl::buffer_t _buffer_owner)
        : buffer_owner(_buffer_owner), buffer({}) {
        buffer[0] = 1;
    }
    constexpr log_handler(noheap::log_impl::owner_impl::buffer_t _buffer_owner,
                          std::span<std::size_t> _buffer)
        : buffer_owner(_buffer_owner) {
        if (_buffer.size() > max_outstream_count)
  ...
  public:
    ...
    void to_console(std::format_string<Args...> format, Args &&...args) const {
        this->log<async>(1, format, std::forward<Args>(args)...);
    }

    template <output_type async = output_type::flush, typename... Args>
    void to_stream(std::size_t it_outstream, std::format_string<Args...> format,
                   Args &&...args) const {
        this->log<async>(buffer.at(it_outstream), format,
                         std::forward<Args>(args)...);
    }
    ...
    void to_all(std::format_string<Args...> format, Args &&...args) const {
        std::for_each(buffer.begin(), buffer.end(), [&](std::size_t outstream) {
            if (!outstream)
                return;
            this->log<async>(buffer.at(outstream), format,
                             std::forward<Args>(args)...);
        });
    }
    template <output_type async = output_type::flush, typename... Args>
    ...
    to_all_with_subowner(noheap::log_impl::owner_impl::buffer_t buffer_subowner,
        ...
        std::for_each(buffer.begin(), buffer.end(), [&](std::size_t outstream) {
            ...
            auto end_it = buffer.begin();
            end_it = std::format_to_n(end_it, noheap::print_impl::buffer_size,
                                      format, std::forward<Args>(args)...)
                         .out;
  ...
  private:
    ...
    constexpr void log(std::size_t outstream,
                       std::format_string<Args...> format,
                       Args &&...args) const {
        switch (async) {
        default:
